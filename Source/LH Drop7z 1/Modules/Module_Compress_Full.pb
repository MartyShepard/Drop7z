...
Module DropPack
    ...
    Procedure.i zCollect_Write(*P.PROGRAM_BOOT)
        ...
        If ( ListSize( Collection() ) >=1 )
            
            ResetList( Collection() )
            
            Debug ""
            Debug "7zArchiv - Schreibe in die Datei: " + *P\szTempFile
            Debug "7zArchiv - List Grösse          : " + Str( ListSize( Collection() ) )
            
            If OpenFile ( DC::#_TempFile, *P\szTempFile )
                ForEach Collection()             
                    WriteStringN(DC::#_TempFile, Collection() )            
                    Debug "         > " + Collection()
                Next    
    ...
    Procedure   zCollect_Files_AllMarked(*P.PROGRAM_BOOT)
        ...
        For Item = 0 To Count
            ...
            If ( Status >= 2 )
                ...
                EndIf
                Delay(1000)            
                AddElement( Collection() ): Collection()  = Path$ + File$
    ...
    Procedure   Delete_Collection()
        ...
        If ( CFG::*Config\AutoClearLt = 1 )
            ...
            If ( ListSize( Collection() ) > 0)
                
                ResetList( Collection() )
                While NextElement( Collection() )
