...
Procedure.i ComboAutoText()
    ...
    If Len(iCBText$) = 0: ProcedureReturn: EndIf
    If Len(iCBText$) = 1: ProcedureReturn: EndIf
    ...
    If (iCBMax = 0): ComboAutoText_Additems(iCBText$): ProcedureReturn
    ...
    ElseIf (iCBMax >= 1)
        ...
        For indexCB = 0 To iCBMax
            ...
            If LCase(iCompare$) = LCase(iCBText$): iNotFound = 1: EndIf
        Next
    
        If (iNotFound = 0): ComboAutoText_Additems(iCBText$): EndIf
        ProcedureReturn
    ...
    If iMax <> 0 
      ...
      For i = 0 To iMax-1
       ...
       FileSize = FileSize(Path$+File$)
        ...
        EndIf
      Next
      SetGadgetText(DC::#Text_002, WhatEver$ + DropLang::GetUIText(12) + FormatFileSize(iFullSize) + DropLang::GetUIText(19) + iMax)
      ProcedureReturn
  EndIf
EndProcedure
...
Procedure.s  _SetAction_Drop()
    ...
    Protected State_ClearFileList$,iMax.i,iDrop_Stuff.s,iMaxCount.i,iElement.i,iStr$,iStO$,Path$,File$,ListName$,iPos.i,iLength.i,FileSize.q
    ...
    iDrop_Stuff.s = EventDropFiles()
    iMax  = CountString(iDrop_Stuff, Chr(10)) + 1
    ...
    iMaxCount  = CountGadgetItems(DC::#ListIcon_001)
            If iMaxCount <> 0 And LCase(State_ClearFileList$) <> "true"
                ...
                For x = 1 To iMax
                    Path$ = GetPathPart(StringField(iDrop_Stuff, x, Chr(10)))  
                    File$ = GetFilePart(StringField(iDrop_Stuff, x, Chr(10)))
            ...
            Else
                For i = 1 To iMax
                    ...
                    Path$ = GetPathPart(StringField(iDrop_Stuff, i, Chr(10)))  
                    File$ = GetFilePart(StringField(iDrop_Stuff, i, Chr(10)))
