...
;     EndProcedure
    ...
    Procedure.s GetFilePath(Title.s = "", InitialPath.s = "",NewDialog = #True,NoDir = #False ,BrowseIncludeFiles = #True, Extension$ = "*",CSIDL = #PB_Default)
        ...
        SetFlags = #BIF_STATUSTEXT|#BIF_UAHINT;|#BIF_EDITBOX
        
        If NewDialog = #True
            SetFlags = SetFlags|#BIF_NEWDIALOGSTYLE|#BIF_USENEWUI 
        EndIf
        
        If BrowseIncludeFiles = #True
            SetFlags = SetFlags|#BIF_BROWSEINCLUDEFILES|#BIF_BROWSEINCLUDEURLS 
        Else
          If (FileSize(InitialPath.s) <> -2) And Not (InitialPath.s = "")
            MessageRequester("Error (2)",#CRLF$+"Path Not Found :"+#CRLF$+InitialPath.s)
               ; RequestEX::MSG("", "Error (2)",#CRLF$+"Path Not Found :"+#CRLF$+InitialPath.s,1,0,"",1)
            EndIf
        EndIf
        
        If NoDir = #True
            SetFlags = SetFlags| #BIF_NONEWFOLDERBUTTON 
        EndIf
...
EndProcedure
    ...
    ;Dies BENÖTIGT ein CoInitialize_(#Null) vor dem Stringgadget und ein CoUninitialize_() beim beenden des Fenster
        Procedure SHAutoComplete(GadgetID.i, FileHistory = #False, PathHistory = #False, UrlHistory = #False, Flags.l = 0)
            ...
            If ( FileHistory = #True )
                SHAutoComplete_(GadgetID(GadgetID), #SHACF_AUTOAPPEND_FORCE_ON | #SHACF_AUTOSUGGEST_FORCE_ON | #SHACF_FILESYSTEM)
                ProcedureReturn
            EndIf
            
            If ( PathHistory = #True )
                SHAutoComplete_(GadgetID(GadgetID), #SHACF_AUTOAPPEND_FORCE_ON | #SHACF_AUTOSUGGEST_FORCE_ON | #SHACF_FILESYS_ONLY)
                ProcedureReturn
            EndIf
            
            If ( UrlHistory = #True )
                SHAutoComplete_(GadgetID(GadgetID), #SHACF_AUTOAPPEND_FORCE_ON | #SHACF_AUTOSUGGEST_FORCE_ON | #SHACF_URLALL)    
                ProcedureReturn
            EndIf
            If ( FileHistory = #False ) And ( UrlHistory = #False )  And  (Flags >= 1)
                SHAutoComplete_(GadgetID(GadgetID),Flags)
                ProcedureReturn
            EndIf                
       EndProcedure     
    ;//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  iResult$ = FFH::GetFilePath("Test", "",#True,#True,#True,"txt")
  Debug iResult$        
