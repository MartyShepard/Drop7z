...
DeclareModule CHD
    ...
    Declare		Compress()
    Declare.i   Compress_SingleFiles(p)
    ...
    Global MassCompress.i = #False
    
    Declare szLineSplit(sText.s, sChar$, RemoveChar.i = #False)
EndDeclareModule
...
Module CHD    
    ...
    Global NewList szSplittetList.s()
    ;****************************************************************************************************************************************************************
...
;     			        
;                         While NextElement( LinkedList() )
;                             Debug "szLineSplit :: DEBUG :: " + LinkedList()
;                         Wend 
;             
;                         ResetList( LinkedList() )    			        
;     			    EndIf
    			    AddElement ( szSplittetList() )    			  
    				While *Source\c
	...
	Procedure	Open_CueFile(szFile.s)
		...
		Debug "Öffne CUE File:"
		Debug "Datei         :" + szFile
		Debug "Länge         :" + Len( szFile )
		Debug "Short (8.3)   :" + GetLongFromShort(szFile)
		Debug "Long Filename :" + GetShortFromLong(szFile)		
			
		If ( OpenFile(DC::#CUEFILE, szFile ,#PB_File_SharedRead|#PB_File_SharedWrite) )
			
			If ( ReadFile(DC::#CUEFILE, szFile) ) 
...
; 										SetGadgetText(DC::#Text_022, SetScrollAreaText(1, BinaryPath$) )
									...
									ISOFileCount()\CueFileList = szFile
								...
								Else
									...
									ISOFileErros()\CueFileList = szFile
	...
	;
	;
	; Öffnen der Iso Datei und die Iso Datei lesen und den Path Überprüfen ob diese auch existiert
	Procedure	Open_IsoFile(File$)
		...
		If ( FileSize( BinaryFile$ ) >= 4096 )
		    If  (ImageCount = 0)
		        ...
		        ; 										SetGadgetText(DC::#Text_022, SetScrollAreaText(1, BinaryPath$) )
		        ;If ( UCase( GetExtensionPart( ISOFileList()\CueFileList ) ) ) = "ISO"
		        ;    ISOFileList()\CueFileList = "Kein Cue File. CD ISO Abbild"
		        ;EndIf    
		        SetGadgetText(DC::#Text_022, SetScrollAreaText(2, GetFilePart( ISOFileList()\CueFileList )))
		        SetGadgetText(DC::#Text_022, SetScrollAreaText(3, GetFilePart( ISOFileList()\BinFileList) ))
		        SetScrollHeight()
	...
	;
    ;
	Procedure	FileList_IsoBin(File$)
		
		Protected nError.i
		nError = Open_IsoFile(File$)
		If ( nError >= 3 )
			ProcedureReturn nError
		EndIf
	...
	Procedure.i  FileList_FormatCheck(szFile.s)
        ...
        Select ( UCase( GetExtensionPart( szFile ) ) )
        	...
        	Case "GDI"
        	    ...
        	    ProcedureReturn 9
        	Case "CHD"
        	    Request::MSG("Format ERR0R", "M.A.M.E. CHDMan Tool"  , " " + GetFilePart(szFile) + " is already Compressed as CHD Image", 2, 2, ProgramFilename(), 0, 0, DC::#_Window_001)
        	    ProcedureReturn 9       	    
        	Default  
        	    
        	    If ( FileSize( szFile ) = -2 )
        	        Request::MSG("Format ERR0R", "M.A.M.E. CHDMan Tool"  ,"Verzeichnisse Können nicht Komprimiert werden", 2, 2, ProgramFilename(), 0, 0, DC::#_Window_001) 
        	        ProcedureReturn  0
        	    EndIf
        	    
        	        
        	    Request::MSG("Format ERR0R", "M.A.M.E. CHDMan Tool"  ,"Can not Compress "+ UCase( GetExtensionPart( szFile )), 2, 2, ProgramFilename(), 0, 0, DC::#_Window_001)         	    
        		ProcedureReturn 9
        EndSelect	    
	...
	Procedure.i	FileList_GetFormat(ItemNr.i)
        ...
        szComboBox.s = GetGadgetText(DC::#String_002)
        
	    If ( Len( szComboBox + File$ ) >= #MAX_PATH )
	            Request::MSG("LENGHT ERR0R", "M.A.M.E. CHDMan Tool"  , "Vorgang kann nicht ausgeführt werden weil der Path zu lang ist" + Chr(32) + "Standard: 255 " + "Verzeichnis und Dateilänge: " + Len(szComboBox + File$), 2, 2, ProgramFilename(), 0, 0, DC::#_Window_001)
	            ProcedureReturn 9   
	    EndIf         
	...
	Procedure	FileList_Marked()
        ...
        Files  = CountGadgetItems(DC::#ListIcon_001)
        ;
        ; Prüfe ob Einträge selektiert sind                
        For  x = 0 To Files-1
            State = GetGadgetItemState(DC::#ListIcon_001, x)            
        ...
        If ( Checked = #True )
            For  x = 0 To Files-1
                ...
                If  ( State = 3 Or State = 2 )   
                    ...
                    szFilePath = GetGadgetItemText(DC::#ListIcon_001, x,0)
                    szFileName = GetGadgetItemText(DC::#ListIcon_001, x,1)
                    If ( Len(szFileName ) > 0 )
                        
                        Select ( UCase( GetExtensionPart( szFileName ) ) )
                            Case "CUE","ISO", "TOC", "GDI"
                                
                                nError = Compress()
                                If ( nError >= 9 )
                                ElseIf ( nError >= 1 ) And ( nError <= 8 )
                                    Continue ; MessagesRequest(nError)
                                Else   
                                    Start_Manager()
                                    ;Delay(50)
                ...
                If ( Len(szFileName ) > 0 )
                    ...
                    Select ( UCase( GetExtensionPart( szFileName ) ) )
                        Case "CUE", "ISO", "TOC", "GDI"
                            ...
                            SetGadgetItemState(DC::#ListIcon_001,x,1|2)                         
                            nError = Compress()
                            SetGadgetItemState(DC::#ListIcon_001,x,0)
                            If ( nError >= 9 )
                            ElseIf ( nError >= 1 ) And ( nError <= 8 )
	...
	Procedure   WindowEv_Clear(*P.PROGRAM_BOOT)
            ...
            If IsWindow( DC::#_Window_005 )                                                                   
               If ( ListSize( szSplittetList() ) > 0)
                   ResetList( szSplittetList() )
                   While NextElement( szSplittetList() )
                       DeleteElement( szSplittetList() )
                   Wend    
                    ResetList( szSplittetList() )                                                   
               EndIf    
               
               ClearStructure(*P, PROGRAM_BOOT)
               FreeMemory(*P) 
	...
	Procedure.s Thread_GetConsoleOut(*P.PROGRAM_BOOT)
	    ...
	    ;Delay(5)   
		CHD::szLineSplit( *P\Logging , #CR$,  #True )
		;Delay(125)
...
; 			EndIf
    			...
    			Repeat
    			    ...
    			    If ReadLen
    			        ...
    			        If ( ReadLen >= MemorySize( *Buffer ) )                            
    			            *Buffer = ReAllocateMemory( *Buffer , ReadLen +1*2);: Delay(5)
   ...
   ;
            ...
            If ( *P\ForceBreak = #False )
                ...
                *P\exitCodeHi      = 0 
                ...
                Else
                    ...
                    Request::MSG("Successfully", "M.A.M.E. CHDMan Tool"  , szMessage$ ,0, 0, "", 0, 0, DC::#_Window_001)
    ...
    ;
    ;
    Procedure .i Compress_SingleFiles(p)
        
        Protected Checked.i, Files.i, State.i, szFileName.s, szFilePath.s, SingleMarked = -1
